...
export class ShowVehicleComponent implements OnInit {
  ...
  private id = null;
  private imgURL = '';
  private vehicleMapURL: SafeResourceUrl;

  constructor(private vehiclesService: VehiclesService, private route: ActivatedRoute, private domSanitizer: DomSanitizer) { }
    ...
    this.vehiclesService.getVehicle(id)
      .subscribe(
        response => {
          ...
          this.vehicleMapURL = this.domSanitizer.bypassSecurityTrustResourceUrl('http://jovanovicmarko.me/maps/vehicle.html?x=' + this.vehicle.parkX + '&y=' + this.vehicle.parkY);
          this.imgURL = '../../../../assets/vehicles/Vehicle_' + this.vehicle.model + '.jpg';
        },
        error => {
          console.log(error);
        }
